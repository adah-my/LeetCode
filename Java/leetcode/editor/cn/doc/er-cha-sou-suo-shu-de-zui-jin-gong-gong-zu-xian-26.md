...
当我们分别得到了从根节点到 *p* 和 *q* 的路径之后，我们就可以很方便地找到它们的最近公共祖先了。显然，*p* 和 *q* 的最近公共祖先就是从根节点到它们路径上的「分岔点」，也就是最后一个相同的节点。因此，如果我们设从根节点到 *p* 的路径为数组 ![\textit{path\_p}\[\] ](./p__textit{path_p}___.png) ，从根节点到 *q* 的路径为数组 ![\textit{path\_q}\[\] ](./p__textit{path_q}___.png) ，那么只要找出最大的编号 *i*，其满足
